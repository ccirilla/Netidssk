 ...
 //                   printf("上传完成\n");
            ...
            if(client[nn_fd].state>0)
            {
                ...
                case TOKEN_PLESE:
                    ...
                    ret = math_token(conn,login_msg.name,login_msg.token);//查数据库，比对token值
                    if(ret == 0)//成功
                    {
                        client[nn_fd].state =2;
                ...
                case OPERATE_Q:
                    ...
                    send(nn_fd,&train,8+train.Len,0);
                    break;
                case DOWNLOAD_PRE://询问是否可以下载
                    strcpy(temp,train.buf);
                    if(client[nn_fd].state < 2)//非已登录状态
                    {
                        train.Len = 0;
                        train.ctl_code = REGISTER_NO;
                    }else
                ...
                case DOWN_MORE_PRE://多点下载请求信息
                    ...
                    }else
                    {
                        ...
                        }else//批准下载
                        {
                            ...
                            ret = get_mqbuf(conn,&mqbuf);//找到别的下载服务器
                            if(ret ==0)
                            {
                ...
                case DOWNLOAD_Q:
                    ...
                    {
                        ret =math_task(down_thread,DOWN_THREAD_NUM,AVG_CLIENT_NUM);//找到最小任务数的线程
                        if(ret == -1)
                        {
                            train.Len = 0;
                            //应该再加个控制码表示服务器繁忙；
